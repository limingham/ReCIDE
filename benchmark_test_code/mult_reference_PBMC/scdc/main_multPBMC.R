...
library(pbmcapply)
library(SCDC)
ER.dgedata_150=readRDS.lbzip2('~/ReCIDE/benchmark测试_最终/mult_reference_PBMC/ref_data/ref_all.rdsFS',n.cores = 300)
...
ER.dgedata_150@meta.data[,'study']='study1'

ER.dgedata_150@meta.data[ER.dgedata_150@meta.data[,'batch'] %in% c("P2","P1","P4","P3","P5","P7","P6","P8"),
                         'study']='study2'

         
##orig.ident在不同情况下需要改

ref_data_in_list=SplitObject(ER.dgedata_150,split.by = "study")
...
QCK<-function(k){
  ...
  pdata_sc <- cbind(cellname = colnames(data_sc), 
                    ...
                    celltype_major=ref_data_in@meta.data[["cell_label"]])
...
saveRDS(eset_sc_qc.list,file = '~/ReCIDE/benchmark测试_最终/mult_reference_PBMC/scdc/SCDC_qc_results.rds')
...
prop_results <- SCDC_ENSEMBLE2(bulk.eset = eset_bulk, sc.eset.list = eset_sc_qc.list, ct.varname = "celltype_major",
                               ...
                               ct.sub =  unique(ER.dgedata_150@meta.data[,'cell_label']), search.length = 0.01, grid.search = T)

saveRDS(prop_results,file="~/ReCIDE/benchmark测试_最终/mult_reference_PBMC/scdc/SCDC_output.rds")
# bulk.eset = eset_bulk
# sc.eset.list = eset_sc_qc.list
# ct.varname = "celltype_major"
# sample = "subjects"
# ct.sub = unique(ER.dgedata_150@meta.data[,'cell_label'])
# grid.search = F 
# search.length = 0.05
# iter.max = 2000
# nu = 1e-04
...
saveRDS(prop_df,file = '~/ReCIDE/benchmark测试_最终/mult_reference_PBMC/scdc/SCDC_prd_df.rds')
