...
library(pbmcapply)
library(SCDC)
ref_data=readRDS.lbzip2('~/ReCIDE/benchmark测试_最终/cross_dataset_PFC/ref_data/ref_all.rdsFS',n.cores=200)
...
pdata_sc <- cbind(cellname = colnames(data_sc), 
                  subjects = as.character(ref_data@meta.data[["subject"]]),
                  celltype_major=as.character(ref_data@meta.data[["major.celltype"]]))
...
prop_results <- SCDC_prop(eset_bulk, eset_sc_qc[["sc.eset.qc"]], ct.varname = "celltype_major", sample = "subjects",
                          ct.sub = unique(ref_data@meta.data[["major.celltype"]]))

saveRDS(prop_results,file = '~/ReCIDE/benchmark测试_最终/cross_dataset_PFC/scdc/SCDC_output.rds')
...
saveRDS(prd_df,file='~/ReCIDE/benchmark测试_最终/cross_dataset_PFC/scdc/SCDC_prd_df.rds')
