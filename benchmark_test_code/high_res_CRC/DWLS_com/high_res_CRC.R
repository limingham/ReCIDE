...
combine.dgedata_150=readRDS.lbzip2(file='~/ReCIDE/benchmark测试_最终/high_res_CRC/ref_data/ref_all.rdsFS',n.cores = 200)
patient_names=unique(combine.dgedata_150@meta.data['PatientTypeID'])


func_mu<-function(i){
  scdata_test<-subset(combine.dgedata_150,PatientTypeID != patient_names[i,1])
  # scdata_test=scdata_test[row.names(ref_com.FC_screen[[i]]),]
  
  scdata_test@meta.data[["usetype"]]<-scdata_test@meta.data[["usetype"]]###########
...
saveRDS(Sig_list,file = '~/ReCIDE/benchmark测试_最终/high_res_CRC/DWLS_com/DWLS_tutorial_new/CRC_com_sig.rds')
...
Sig_list <- readRDS("~/ReCIDE/benchmark测试_最终/high_res_CRC/DWLS_com/CRC_com_sig.rds")
...
sep_solDWLS<-pbmclapply(1:length(EXP),fun_DWLS_in,mc.cores = 55)
names(sep_solDWLS)<-names(EXP)
# }
#save(sep_solDWLS)
# sep_solDWLS[[156]]<-NULL
saveRDS(sep_solDWLS,file='~/ReCIDE/benchmark测试_最终/high_res_CRC/DWLS_com/DWLS_com_output.rds')
...
saveRDS(prd_com,file='~/ReCIDE/benchmark测试_最终/high_res_CRC/DWLS_com/prd_com_CRC.rds')
