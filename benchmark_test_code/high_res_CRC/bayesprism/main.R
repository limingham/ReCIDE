...
combined.dgedata_150=readRDS.lbzip2('~/ReCIDE/benchmark测试_最终/high_res_CRC/ref_data/ref_all.rdsFS',n.cores = 200)
# combined.dgedata_150.list<- SplitObject(combined.dgedata_150, split.by = "PatientTypeID")
# combined.dgedata_150.list=combined.dgedata_150.list[sort(names(combined.dgedata_150.list))]
patient_names=unique(combined.dgedata_150@meta.data['PatientTypeID'])
...
func_makerPrismObject<-function(x){
  data_ref=subset(combined.dgedata_150,PatientTypeID != patient_names[x,1])
  # data_ref=data_ref[row.names(ref_com.FC_screen[[x]]),]
  
  bulk_seurat<-subset(combined.dgedata_150,PatientTypeID == patient_names[x,1])
  ...
  cell.type.labels<-data_ref@meta.data['usetype']
...
myPrism.list<-pbmclapply(1:nrow(patient_names),func_makerPrismObject, mc.cores = 1)
names(myPrism.list)<-patient_names[,1]

saveRDS(myPrism.list,file='~/ReCIDE/benchmark测试_最终/high_res_CRC/bayesprism/CRC_myPrism.list.rds')

rm(list=ls())
gc()

myPrism.list <- readRDS("~/ReCIDE/benchmark测试_最终/high_res_CRC/bayesprism/TNBC_myPrism.list.rds")
# myPrism.list=myPrism.list[["value"]]
...
saveRDS(BayesPrism.results,file = '~/ReCIDE/benchmark测试_最终/high_res_CRC/bayesprism/BayesPrism_output.rds')




BayesPrism.results <- readRDS("~/ReCIDE/benchmark测试_最终/high_res_CRC/bayesprism/BayesPrism_output.rds")
names(BayesPrism.results)=names(TNBC_myPrism.list)
...
prd_df=df_merge
saveRDS(prd_df,file = '~/ReCIDE/benchmark测试_最终/high_res_CRC/bayesprism/prd_df.rds')
