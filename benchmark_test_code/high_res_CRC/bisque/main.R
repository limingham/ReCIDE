...
for (j in 1:nrow(patient_names)) {
  bulk_seurat<-subset(combine.dgedata_150,PatientTypeID == patient_names[j,1])
...
function_bisque_in<-function(i){
  ...
  individual.labels<-sc_seurat@meta.data['PatientTypeID']
  cell.type.labels<-sc_seurat@meta.data['usetype']
...
results_bisque<-pbmclapply(1:nrow(patient_names),function_bisque_in,mc.cores = 1)

names(results_bisque)<-patient_names[,1]

saveRDS(results_bisque,file = '~/ReCIDE/benchmark测试/high_res_CRC/bisque/bisque_tutorial/bisuqe_output.rds')
...
saveRDS(df_merge,file='~/ReCIDE/benchmark测试/high_res_CRC/bisque/bisque_tutorial/prd_bisque.rds')
prd_df=df_merge
