...
combine.dgedata_150=readRDS.lbzip2('~/ReCIDE/benchmark测试_最终/high_res_CRC/ref_data/ref_all.rdsFS',n.cores = 200)
...
func_in<-function(f){
  ...
  ref_data<-subset(combine.dgedata_150,PatientTypeID != patient_names[f,1])
  ...
  pdata_sc <- cbind(cellname = colnames(data_sc), 
                    subjects = as.character(ref_data@meta.data[["PatientTypeID"]]),
                    celltype_major=as.character(ref_data@meta.data[["usetype"]]))
  ...
  prop_results <- SCDC_prop(eset_bulk, eset_sc_qc[["sc.eset.qc"]], ct.varname = "celltype_major", sample = "subjects",
                            ct.sub = unique(ref_data@meta.data[["usetype"]]))
  
  return(prop_results)
}

prop_results.list=pbmclapply(1:nrow(patient_names),func_in,mc.cores = 3)
names(prop_results.list)=patient_names[,1]

saveRDS(prop_results.list,file = '~/ReCIDE/benchmark测试_最终/high_res_CRC/scdc/SCDC_output.rds')

prd_df=as.data.frame(t(prop_results[["prop.est.mvw"]]))

# key_df <- readRDS("~/scRNA_Seq_data/kidney/benchmark/EXP_and_KEY/KEY_kidney.rds")
...
saveRDS(prd_df,file='~/ReCIDE/benchmark测试_最终/high_res_CRC/scdc/SCDC_prd_df.rds')



SCDC_output<- readRDS("~/ReCIDE/benchmark测试_最终/high_res_CRC/scdc/SCDC_output.rds")
...
saveRDS(de_1,file='~/ReCIDE/benchmark测试_最终/high_res_CRC/scdc/prd_df.rds')
