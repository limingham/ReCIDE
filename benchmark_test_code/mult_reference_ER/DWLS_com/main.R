...
scdata_test=readRDS.lbzip2(file='~/ReCIDE/benchmark测试/mult_reference_ER新/ref_data/ER_mult.150FC_use.rdsFS',n.cores = 200)
...
# Sig <- as.data.frame(lapply(Sig, as.numeric))
Sig <- dplyr::mutate_all(Sig, as.numeric)
saveRDS(Sig,file = '~/ReCIDE/benchmark测试/mult_reference_ER新/DWLS_com/ER_com_sig.rds')
# save(Sig_list,file = '~/SWORD/前聚类测试/CRC_Sig_list09.RData')


EXP<- readRDS("~/ReCIDE/benchmark测试/mult_reference_ER新/EXP_and_KEY/EXP.rds")

# Sig_list<- readRDS("~/ReCIDE/benchmark测试/cross_dataset_PFC/ref_data/ref_com.FC_screen.rds")
Sig_list <- readRDS("~/ReCIDE/benchmark测试/mult_reference_ER新/DWLS_com/ER_com_sig.rds")
...
saveRDS(sep_solDWLS,file='~/ReCIDE/benchmark测试/mult_reference_ER新/DWLS_com/DWLS_com_output.rds')
...
saveRDS(prd_com,file='~/ReCIDE/benchmark测试/mult_reference_ER新/DWLS_com/prd_com_PBMC.rds')
...
sum(abs(prd_com-key_df))/ncol(key_df)