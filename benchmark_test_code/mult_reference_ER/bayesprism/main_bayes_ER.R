...
ref_seurat=readRDS.lbzip2('~/ReCIDE/benchmark测试_最终/mult_reference_ER新/ref_data/ref_all.rdsFS',n.cores = 200)
...
cell.type.labels<-ref_seurat@meta.data['usetype']
...
saveRDS(bp.res,file = '~/ReCIDE/benchmark测试_最终/mult_reference_ER新/bayesprism/Bayes_output.rds')


theta=as.data.frame(t(theta))
colnames(theta)=names(EXP)
saveRDS(theta,file = '~/ReCIDE/benchmark测试_最终/mult_reference_ER新/bayesprism/Bayes_prd_df.rds')
