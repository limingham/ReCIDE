...
combine.dgedata_150=readRDS.lbzip2(file='~/ReCIDE/benchmark测试/high_res_TNBC/ref_data/ref_all.rdsFS',n.cores = 200)
gene_sum=apply(combine.dgedata_150@assays[["RNA"]]@counts,1,sum)

# gene_10=table(gene_sum>10)


combine.dgedata_150=combine.dgedata_150[names(gene_sum[gene_sum>10]),]

patient_names=unique(combine.dgedata_150@meta.data['batch'])


func_mu<-function(i){
  scdata_test<-subset(combine.dgedata_150,batch != patient_names[i,1])
  # scdata_test=scdata_test[row.names(ref_com.FC_screen[[i]]),]
  
  scdata_test@meta.data[["usetype"]]<-scdata_test@meta.data[["celltype_subset"]]###########
...
saveRDS(Sig_list,file = '~/ReCIDE/benchmark测试_最终/high_res_TNBC/DWLS_com/DWLS_com_cosg100_200/TNBC_com_sig.rds')
...
Sig_list <- readRDS("~/ReCIDE/benchmark测试_最终/high_res_TNBC/DWLS_com/TNBC_com_sig.rds")
...
saveRDS(sep_solDWLS,file='~/ReCIDE/benchmark测试_最终/high_res_TNBC/DWLS_com/DWLS_com_output.rds')
...
saveRDS(prd_com,file='~/ReCIDE/benchmark测试_最终/high_res_TNBC/DWLS_com/prd_com_TNBC.rds')
