...
data_mean_filter=data_mean[data_mean[,'true_prop']>0.1,]
# data_mean_filter=subset(data_mean,true_prop<0.1 & true_prop>0.02)


plotdata=subset(combine_data,ident %in% row.names(data_mean_filter))

#####线性回归并绘制柱状图
slope_c=c(lm(plotdata[plotdata[,'methods'] == 'ReCIDE-DWLS','true_prop'] ~ plotdata[plotdata[,'methods'] == 'ReCIDE-DWLS','prd_prop'] + 0)[["coefficients"]][1],
...
###3，4.5
# plot.bar <- reorder(methods,value)
...
p2=ggplot(plotdata, aes(prd_prop, true_prop)) +
  ...
  # ggtitle("DWLS-Minor")+
  ggtitle("DWLS-Major")+
  geom_point(aes(fill = methods, color = methods, shape=methods),size = 1, alpha = 0.7)+
  ...
  coord_cartesian(xlim = c(0.025, 0.7), ylim = c(0.025, 0.7))+
