...
combine.dgedata_150=readRDS.lbzip2(file='~/ReCIDE/benchmark测试_最终/deone_PAN/ref_data/ref_all.rdsFS',n.cores = 200)
patient_names=unique(combine.dgedata_150@meta.data['donor_id'])


func_mu<-function(i){
  scdata_test<-subset(combine.dgedata_150,donor_id != patient_names[i,1])
  
  scdata_test@meta.data[["usetype"]]<-scdata_test@meta.data[["cell_label"]]###########
...
Sig_list=pbmclapply(1:nrow(patient_names),func_mu,mc.cores=12)
names(Sig_list)<-patient_names[,1]

saveRDS(Sig_list,file = '~/ReCIDE/benchmark测试_最终/deone_PAN/DWLS_com/PFC_com_sig.rds')
...
Sig_list <- readRDS("~/ReCIDE/benchmark测试_最终/deone_PAN/DWLS_com/PFC_com_sig.rds")
...
saveRDS(sep_solDWLS,file='~/ReCIDE/benchmark测试_最终/deone_PAN/DWLS_com/DWLS_com_output.rds')
...
saveRDS(prd_com,file='~/ReCIDE/benchmark测试_最终/deone_PAN/DWLS_com/prd_com_PFC.rds')
