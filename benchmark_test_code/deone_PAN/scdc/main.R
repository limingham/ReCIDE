...
func_in<-function(f){
  ...
  ref_data<-subset(combine.dgedata_150,donor_id != patient_names[f,1])
  ...
  pdata_sc <- cbind(cellname = colnames(data_sc), 
                    subjects = as.character(ref_data@meta.data[["donor_id"]]),
                    celltype_major=as.character(ref_data@meta.data[["cell_label"]]))
  ...
  prop_results <- SCDC_prop(eset_bulk, eset_sc_qc[["sc.eset.qc"]], ct.varname = "celltype_major", sample = "subjects",
                            ct.sub = unique(ref_data@meta.data[["cell_label"]]))
...
saveRDS(prop_results.list,file = '~/ReCIDE/benchmark测试_最终/deone_PAN/scdc/SCDC_output.rds')



SCDC_output <- readRDS("~/ReCIDE/benchmark测试_最终/deone_PAN/scdc/SCDC_output.rds")
...
saveRDS(de_1,file='~/ReCIDE/benchmark测试_最终/deone_PAN/scdc/prd_df.rds')
