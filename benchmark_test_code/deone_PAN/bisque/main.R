...
combine.dgedata_150=readRDS.lbzip2('~/ReCIDE/benchmark测试/deone_PAN/ref_data/ref_all.rdsFS',n.cores = 200)
patient_names=unique(combine.dgedata_150@meta.data['donor_id'])

bulk.mtx<-0
for (j in 1:nrow(patient_names)) {
  bulk_seurat<-subset(combine.dgedata_150,donor_id == patient_names[j,1])
...
function_bisque_in<-function(i){
  ...
  individual.labels<-sc_seurat@meta.data['donor_id']
  cell.type.labels<-sc_seurat@meta.data['cell_label']
...
saveRDS(results_bisque,file = '~/ReCIDE/benchmark测试/deone_PAN/bisque/bisque_tutorial/bisuqe_output.rds')
...
saveRDS(df_merge,file='~/ReCIDE/benchmark测试/deone_PAN/bisque/bisque_tutorial/prd_bisque.rds')
prd_df=df_merge
key_df <- readRDS("~/ReCIDE/benchmark测试/deone_PAN/EXP_and_KEY/KEY.rds")
