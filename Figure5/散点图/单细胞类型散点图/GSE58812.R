...
ggplot(data_matrix, aes(surtime, PVLs1)) +
  ...
  geom_hline(yintercept = 0.0016,linetype=3)+
...
FT=c()
k=1
for (i in seq(0,0.035,0.0001)) {
  data_matrix1=data_matrix[data_matrix[,'PVLs1']>i,]
  data1=table(data_matrix1[,5])
  
  data_matrix2=data_matrix[data_matrix[,'PVLs1']<i,]
  data2=table(data_matrix2[,5])
...
seq(0,0.035,0.0001)[17]
# proportion= 0.043
# p-value = 0.003135
data_matrix1=data_matrix[data_matrix[,'PVLs1']>0.0016,]
table(data_matrix1[,5])

data_matrix2=data_matrix[data_matrix[,'PVLs1']<0.0016,]
table(data_matrix2[,5])


data <- matrix(c(74,27, 1, 2), nrow = 2) 
colnames(data) <- c("组1", "组2") 
rownames(data) <- c("暴露", "未暴露")
