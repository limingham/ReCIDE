...
gene_exp2<-as.data.frame(t(prd_after['T_cells_c3_CD4__Tfh_CXCL13',]*prd_after['PVL_Immature_s1',]/prd_after['PVL_Differentiated_s3',]/pvl_all))
...
result_p=c()
aa=1
for (k in seq(0.0001,1.5,0.0002)) {


##<=的话小于的被cut掉，成为TRUE
cutoff= (gene_exp2[,1]<=k)#以中位数为界分为高表达及低表达组
...
surv_diff <- pairwise_survdiff(Surv(surtime, surstat) ~ ak2, data = gene_exp)
# p.value  <- pairwise_survdiff(Surv(surtime, surstat) ~ ak2, data = gene_exp)
p.val = surv_diff[["p.value"]]
...
##<=的话小于的被cut掉，成为TRUE
cutoff= (gene_exp2[,1]<=0.0125)#以中位数为界分为高表达及低表达组
